...
def _fixture_pack() -> FixturePack:
    return FixturePack(
        cases=[
            ...
            FixtureCase("double_ok", "double precision", 1.5, "float64", expected_coerced=1.5),
            FixtureCase(
                "numeric_ok", "numeric(20,5)", Decimal("3.14"), "float64", expected_coerced=3.14
            ),
            ...
            FixtureCase("array_ok", "integer[]", [1, 2, 3], "string", expected_coerced="[1,2,3]"),
            FixtureCase("bytea_ok", "bytea", b"\x01\x02\xff", "string", expected_coerced="0102ff"),
            FixtureCase("inet_ok", "inet", "192.168.0.1", "string", expected_coerced="192.168.0.1"),
            FixtureCase(
                "interval_ok",
                "interval",
                dt.timedelta(seconds=30, microseconds=500),
                "int64",
            ...
            FixtureCase(
                "timestamp_naive",
                "timestamp",
                dt.datetime(2026, 5, 11, 18, 23, 45),
                "datetime64[ns]",
            ...
            FixtureCase(
                "timestamptz_utc",
                "timestamptz",
                dt.datetime(2026, 5, 11, 12, 0, 0, tzinfo=dt.timezone(dt.timedelta(hours=2))),
                "datetime64[ns]",
                expected_coerced=dt.datetime(2026, 5, 11, 10, 0, 0),
            ),
            FixtureCase(
                "time_ok", "time", dt.time(14, 30, 5), "string", expected_coerced="14:30:05"
            ),
            ...
            FixtureCase(
                "interval_over_int32",
                "interval",
                dt.timedelta(seconds=3548, microseconds=933426),
                "int64",
                roundtrip=False,
                raises=Row64CodecOverflowError,
                raises_stage="coerce",
            ...
            FixtureCase(
                ...
                "float64",
                roundtrip=False,
                raises=NumericPrecisionLossError,
                raises_stage="coerce",
            ),
        ]
    )
