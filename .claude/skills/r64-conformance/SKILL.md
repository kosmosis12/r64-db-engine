...
description: >
  ...
  nine parameterized checks per registered dialect (registry admission,
  ...
  checksum, and an optional zero-copy dev-serve gate asserting
  copied_columns=0). Emits a two-form evidence pack; the exit code is the
...
| 9 | `zero_copy_serve_gate` | a column was copied instead of mapped, or the warm pass still decoded |
...
2. **Summary table.** Nine rows. If there are not nine, the battery shrank —
...
5. **Environment.** `pyarrow` owns the IPC block layout and `pandas` decides
   ...
   worth chasing.
6. **Sanity floor.** Confirm `artifact.rows` and `artifact.blocks` are what you
...
3. **Write the failing fixture in `tests/factory/test_battery.py`.** A check
   with no proof it can FAIL is a green light with no bulb behind it. This step
   is not optional and is not follow-up work.
4. Append it to the ordered list asserted by `test_every_check_ran`.
